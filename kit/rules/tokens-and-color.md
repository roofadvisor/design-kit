...
4. **One CI should enforce all of it** — this repo has **no such workflow yet**. Until one exists, `validate_tokens` + `validate_contrast` + `validate_component_spec` run only when invoked (`node ${CLAUDE_PLUGIN_ROOT}/kit/scripts/accuracy_report.mjs` bundles them), and nothing blocks a drift-introducing merge; wiring the workflow is part of the design-qa skill's job when invoked.
