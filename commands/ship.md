...
1. Quality gate (must be fully green before continuing):
   - `node ${CLAUDE_PLUGIN_ROOT}/kit/scripts/accuracy_report.mjs` — report the real N/N.
   - `node ${CLAUDE_PLUGIN_ROOT}/kit/scripts/verify_responsive.mjs examples` — no overflow at 280/320/414.
   - `python3 ${CLAUDE_PLUGIN_ROOT}/kit/scripts/check_no_emoji.py` — UI + taste + instruction surface.
...
2. Release checklist (per the project release rule):
   ...
   - Confirm `${CLAUDE_PLUGIN_ROOT}/kit/examples/apple-home` stays gitignored (local IP).
